/**
 * Copyright (c) 2020-2025 Toyomasa Watarai
 *
...
RCS620S rcs620s(RCS620S_UART_TX, RCS620S_UART_RX, uart0);
...
int main() {
    ...
    if (rcs620s.initDevice() == 0) {
        printf("[ERROR] RC-S620/S device initialize error!\n");
        return -1;
    }
    ...
    while (1) {
        ...
        || rcs620s.polling(SAPICA_SYSTEM_CODE)
        || rcs620s.polling(PASPY_SYSTEM_CODE) ) {
        ...
        if (rcs620s.polling(RYUTO_SYSTEM_CODE)) {
            char info[80];
